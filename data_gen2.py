...
class SpaceKnowledgeExtractor:
    ...
    def extract_entity_attributes(self, entity: Dict[str, Any], full_text: str) -> List[Dict[str, str]]:
        ...
        try:
            ...
            for attr_name, attr_value in attr_dict.items():
                if attr_value:  # 过滤空值
                    attributes.append({
                        "entity": entity["name"],  # 明确关联实体
                        "attribute": attr_name,
                        "value": attr_value,
                        "attribute_type": "",  # 保留字段供后续使用
                        "source_context": self._find_attribute_context(full_text, attr_value)
                    })
            return attributes
        except json.JSONDecodeError:
            print("属性解析失败，采用备用解析策略")
            return self._fallback_attribute_parsing(response, entity["name"])
    ...
    def _fallback_attribute_parsing(self, raw_response: str, entity_name: str) -> List[Dict[str, str]]:
        """当JSON解析失败时的备用解析方法（添加实体关联）"""
        attributes = []
        # 尝试识别key: value格式
        ...
        for line in lines:
            if ':' in line:
                ...
                if attr and value:
                    attributes.append({
                        "entity": entity_name,  # 关联实体名称
                        "attribute": attr,
                        "value": value,
                        "attribute_type": "",  # 保留字段
                        "source_context": ""   # 无法获取上下文
                    })
        return attributes
...
class InstructionDatasetGenerator:
    ...
    def process_document(self, doc_path: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        """处理单个航天领域文档，返回知识抽取结果和指令数据"""
        # 读取文档内容
        ...
        for entity in entities:
            ...
            knowledge_graph.append(kg_node)
        
        # 保存知识抽取结果到单独文件
        self.save_knowledge_graph(knowledge_graph, doc_path)
        ...
        for kg in knowledge_graph:
            ...
            # 多项选择题
            if len(kg["entity_attributes"]) >= 3:
                dataset.append(self._generate_multi_choice(kg))
            
            # 完形填空
            if kg["entity_context"]:  # 确保有上下文
        ...
        return knowledge_graph, dataset
    
    def save_knowledge_graph(self, knowledge_graph: List[Dict], doc_path: str):
        """保存知识抽取结果到JSON文件"""
        ...
        file_name = f"knowledge_graph_{base_name}_{timestamp}.json"
        
        # 保存为格式化的JSON
        with open(file_name, 'w', encoding='utf-8') as f:
            json.dump(knowledge_graph, f, indent=2, ensure_ascii=False)
        
        print(f"已保存知识抽取结果到: {file_name}")
    ...
    def _generate_single_qa(self, kg: Dict) -> Dict:
        ...
        # 构建完整答案
        answer = f"{kg['entity']}是{kg['entity_type']}，主要信息包括：\n"
        for attr in kg["entity_attributes"]:
        ...
        return {
            ...
            "input": "",
            "output": answer.strip(),
            "source": kg["entity_context"][:500]  # 截断长上下文
        }
    
    def _generate_multi_choice(self, kg: Dict) -> Dict:
        """生成多项选择题"""
        # 随机选择3个真实属性和1个干扰项
        real_attrs = kg["entity_attributes"][:3]
    ...
    def _generate_multi_turn(self, kg: Dict) -> List[Dict]:
        ...
        conversations.append({
            "role": "assistant",
            "content": f"{kg['entity']}是{kg['entity_type']}，主要应用于{kg['entity_attributes'][0]['value'] if kg['entity_attributes'] else '航天领域'}"
        })
        
        # 后续轮次：基于属性提问
        for i, attr in enumerate(kg["entity_attributes"][:3]):  # 限制轮次
            conversations.append({
                "role": "user",
    ...
    def _generate_cloze(self, kg: Dict) -> Dict:
        """生成完形填空数据"""
        # 构建含空缺的文本
        context = kg["entity_context"]
        if not context:
            return None
        
        # 随机选择一个属性值进行遮盖
        if kg["entity_attributes"]:
            target_attr = kg["entity_attributes"][0]
    ...
    def _generate_info_extraction(self, kg: Dict) -> Dict:
        ...
        structured_info = {
            "entity": kg["entity"],
            "type": kg["entity_type"],
            "attributes": {attr["attribute"]: attr["value"] for attr in kg["entity_attributes"]}
        }
        
        return {
            "type": "info_extraction",
            "instruction": instruction.strip(),
            "input": kg["entity_context"][:500],
    ...
    def _generate_summarization(self, text: str, kg: List[Dict]) -> Dict:
        ...
        for entity in kg[:3]:  # 摘要前3个关键实体
            summary += f"- {entity['entity']}：{entity['entity_type']}，"
            if entity["entity_attributes"]:
                summary += f"关键属性：{entity['entity_attributes'][0]['attribute']}={entity['entity_attributes'][0]['value']}\n"
...
if __name__ == "__main__":
    ...
    # 处理航天文档（示例路径）
    knowledge_graph, dataset = generator.process_document("天问一号技术文档.txt")
    
    # 打印知识抽取结果示例
    print("\n知识抽取结果示例：")
    if knowledge_graph:
        sample = knowledge_graph[0]
        print(f"实体: {sample['entity']}")
        print(f"类型: {sample['entity_type']}")
        print(f"上下文: {sample['entity_context'][:100]}...")
        print("属性:")
        for attr in sample["entity_attributes"][:3]:
            print(f"  - {attr['attribute']}: {attr['value']}")
    
    # 保存指令数据为llama-factory兼容格式
